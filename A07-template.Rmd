...
Trips <-
  Trips %>%
  mutate(time = as.numeric(edate - sdate))
...
#box & wisker plot
ggplot(Trips_filtered, aes(x = client, y = hours)) +
  geom_boxplot(fill = "#81F1E7") +
  labs(y = "dur/60") +
  theme_linedraw() +
  ylim(0, 30)
...
```{r}
### Your Code Here
ggplot(Trips_filtered, aes(x = yday(sdate))) +
  geom_density(adjust = 1L, fill = "#6B6B6B") +
  theme_minimal() +
  labs(x = "day of year")
```


Examining the graph, it looks like there was more usage of the bikes towards the begining of the fourth quarter. It looks like the season and weather may have an effect on the number of bikes used.
...
```{r}
### Your Code Here
ggplot(Trips_filtered, aes(x = day(sdate))) +
  geom_density(adjust = 1L, fill = "#6B6B6B") +
  theme_minimal() +
  labs(x = "day of week")
```
...
From the "Day of the week graph," there seems to be a steady downwards trending line, but each week seems to stay pretty constant. As the weeks go on, the amount of trips fall with the addition of a couple peaks.
...
```{r}
### Your Code Here
ggplot(Trips_filtered, aes(x = hours)) +
  geom_density(adjust = 1L, fill = "#6B6B6B") +
  theme_minimal() +
  xlim(0, 24) +
  labs(x = "hour of day")
...
For the "hour of the day" graph, I can see that that the peak use of the peak use of the bikes are around 6 am. Later in the day the density of trips fall and there is mimial usage frpm 10pm-12:00am.

### Minute of Hour

```{r}
### Your Code Here
ggplot(Trips_filtered, aes(x = minute(sdate))) +
  geom_density(adjust = 1L, fill = "#6B6B6B") +
  theme_minimal() +
  labs(x = "minute of hour") 
```


Looking at the "minute of hour" graph, we can see the steady use of the bike throughout the whole hour. The general slope remains constant. 
...
groups <-
  Trips_filtered %>%
  group_by(hours, week = wday(sdate), client) %>%
  summarize(count = n())

ggplot(groups, aes(x = hours, y = count, group = client)) +
  geom_line(aes(color = client)) +
  facet_wrap(~ week, nrow = 1) +
  labs(x = "hour", y = "count")
```

Looking at the graph, a trend that can be seen is that there seems to be a lot more registered users using making trips at arounf the same time each day and there seems to be more casual users towards the night but over all the casual count seems to stay around the same.
...
### My code here
start_to_end <-
  inner_join(Trips, PairDistances)
...
both_graphs <-
  ride_dis +
  geom_density(data = PairDistances, aes(x = distance), fill = "LightSkyBlue", alpha = 0.5) +
...
Stations and Trips are used for different purposes. Stations describes the names of each station and in Trips, it describes beginning and end stations of each trip. In joining Simple and Simple2, the name variable in Stations was renamed to be sstation, as the starting place of the distance. Simple2 was renamed to estatation to take place of the ending coordinates of the trip. This would put a clear difference from one to the other making it easier to use the data.
